...
#     # for down-sampled image
#     # def sample_3D_w_trained_model(self, trained_model_filename, ground_truth_image_file, motion_image_file,  slice_range, save_file,save_gt_motion = None, save_samples = False, background_cutoff = None, maximum_cutoff = None,  normalize_factor = 1000):
...
#     #     final_image = np.zeros(self.image_size_3D)

#     #     print('ground truth file: ', ground_truth_image_file)
#     #     gt = nb.load(ground_truth_image_file)
#     #     gt_img = gt.get_fdata()
#     #     gt_img = gt_img[:,:,slice_range[0]: slice_range[1]]
#     #     gt_img = Data_processing.cutoff_intensity(gt_img, cutoff_low = background_cutoff, cutoff_high = maximum_cutoff)
#     #     nb.save(nb.Nifti1Image(gt_img, gt.affine), os.path.join(os.path.dirname(save_file) , 'gt.nii.gz'))
...
#     #             pred_img = self.ema.ema_model.sample(condition = data_condition, batch_size = self.batch_size ,return_all_timesteps = save_samples, save_all_timesteps_ref = save_all_timesteps_ref )  
#     #         else:
#     #             pred_img = self.ema.ema_model.sample(batch_size = self.batch_size, return_all_timesteps = save_samples, save_all_timesteps_ref = save_all_timesteps_ref)   
...
#     #         motion_img = Data_processing.cutoff_intensity(motion_img, cutoff_low = background_cutoff, cutoff_high = maximum_cutoff)
#     #         motion_img_save = nb.Nifti1Image(motion_img, gt.affine)
#     #         nb.save(motion_img_save, os.path.join(os.path.dirname(save_file), 'motion.nii.gz'))
