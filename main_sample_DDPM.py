...
save_folder = os.path.join('/mnt/camca_NAS/diffusion_ct_motion/models', trial_name, 'pred_images_simulated'); os.makedirs(save_folder, exist_ok=True)
...
n = ff.get_X_numbers_in_interval(total_number = x0_list.shape[0],start_number = 6,end_number = 7, interval = 20) # each case has 20 motion samples
...
for i in range(0,x0_list.shape[0]):
    ...
    save_folder_case = os.path.join(save_folder, patient_id, patient_subid, motion_name, 'epoch-' + str(epoch))
